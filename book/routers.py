...
async def create_book(book: BookModelRequest = Body(), book_data_access_layer: DataAccessLayer = Depends(get_book_data_access_layer)) -> BookModelResponse:
    created_book = await book_data_access_layer.create_book(book.name, book.author, book.release_year, book.isbn)
    print(create_book)
    return created_book


@router.put("/books/{id}", tags=["put book"])
async def update_book(id: str = Query(description="book id", ), book: BookModelRequest = Body(),
...
@router.get("/books", response_model=List[BookModelResponse], tags=["get books"])
...
async def get_book(id: str = Query(description="book id", ), book_dal: DataAccessLayer = Depends(get_book_data_access_layer)) -> BookModelResponse:
